...
public:
    ...
    MyState(array<value_type, 2> data): state(data) {    }
    ...
    static MyState dfdt(MyState& s){
        return MyState {array<value_type, 2> {s[1], -1*sin(s[0])}};
    }
};
...
void EulerSolve() {
    ifstream fin;
    fin.open("data.txt");
    double X0, Y0, omega, dt;
    long long numb_of_tests;
    fin>>X0>>Y0>>omega>>dt>>numb_of_tests;
    fin.close();
    ...
    MyState<double> init(std::array<double,2>{X0, Y0});

    for (long long i = 1; i < numb_of_tests; i++) {
        init = generic_euler<MyState<double>>(init, dt);
        Xdata[i] = init[0];
        Vdata[i] = init[1];
    }
