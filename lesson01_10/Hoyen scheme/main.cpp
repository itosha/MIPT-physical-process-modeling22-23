...
void HoyeneSch(double *X, double *V, double x0, double v0, double dt, double constant) {
    HcEulerSch(X, V, x0, v0, dt, constant);
    double cup = *V;
    *V = fma(dt, fma(constant/2, (x0 + *X), 0.d), v0);
    *X = fma(dt/2, v0 + cup, x0);
}
...
void dataCount(double x0, double v0, double omega, double dt, int numb_of_tests) {
    ...
    {
        HoyeneSch(&Xdata[i], &Vdata[i], Xdata[i-1], Vdata[i-1], dt, constant);
    }
